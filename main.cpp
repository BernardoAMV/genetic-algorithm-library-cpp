...
int main() {
    ...
    std::vector<GeneBounds> bounds = {
        std::pair<double, double>{0, MAXFLOAT},   // x1
        std::pair<double, double>{0, MAXFLOAT},   // x2
    };
    ...
    auto constraint2 = [](const std::vector<double>& x) {
        return (2 * x[0] + x[1]) - 4.0;
    };

    auto constraint3 = [](const std::vector<double>& x) {
        return (x[0] + x[1]) - 15;
    };

    std::vector<GeneticAlgorithm::ConstraintFunction> constraints = { constraint1, constraint2, constraint3 };
    ...
    double check1 = values[0] + values[1];
